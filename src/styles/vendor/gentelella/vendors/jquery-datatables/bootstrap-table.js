  ...
  */
  ...
  var CONSTANTS = {
    3: {
      ...
      html: {
        toolbarDropdown: ['<ul className="dropdown-menu" role="menu">', '</ul>'],
        toolbarDropdownItem: '<li className="dropdown-item-marker" role="menuitem"><label>%s</label></li>',
        toolbarDropdownSeparator: '<li className="divider"></li>',
        pageDropdown: ['<ul className="dropdown-menu" role="menu">', '</ul>'],
        pageDropdownItem: '<li role="menuitem" className="%s"><a href="#">%s</a></li>',
        dropdownCaret: '<span className="caret"></span>',
        pagination: ['<ul className="pagination%s">', '</ul>'],
        paginationItem: '<li className="page-item%s"><a className="page-link" aria-label="%s" href="javascript:void(0)">%s</a></li>',
        icon: '<i className="%s %s"></i>',
        inputGroup: '<div className="input-group">%s<span className="input-group-btn">%s</span></div>',
        searchInput: '<input className="%s%s" type="text" placeholder="%s">',
        searchButton: '<button className="%s" type="button" name="search" title="%s">%s %s</button>',
        searchClearButton: '<button className="%s" type="button" name="clearSearch" title="%s">%s %s</button>'
      }
    },
    4: {
      classes: {
      ...
      html: {
        toolbarDropdown: ['<div className="dropdown-menu dropdown-menu-right">', '</div>'],
        toolbarDropdownItem: '<label className="dropdown-item dropdown-item-marker">%s</label>',
        pageDropdown: ['<div className="dropdown-menu">', '</div>'],
        pageDropdownItem: '<a className="dropdown-item %s" href="#">%s</a>',
        toolbarDropdownSeparator: '<div className="dropdown-divider"></div>',
        dropdownCaret: '<span className="caret"></span>',
        pagination: ['<ul className="pagination%s">', '</ul>'],
        paginationItem: '<li className="page-item%s"><a className="page-link" aria-label="%s" href="javascript:void(0)">%s</a></li>',
        icon: '<i className="%s %s"></i>',
        inputGroup: '<div className="input-group">%s<div className="input-group-append">%s</div></div>',
        searchInput: '<input className="%s%s" type="text" placeholder="%s">',
        searchButton: '<button className="%s" type="button" name="search" title="%s">%s %s</button>',
        searchClearButton: '<button className="%s" type="button" name="clearSearch" title="%s">%s %s</button>'
      }
    ...
    5: {
      ...
      html: {
        ...
        paginationItem: '<li className="page-item%s"><a className="page-link" aria-label="%s" href="javascript:void(0)">%s</a></li>',
        icon: '<i className="%s %s"></i>',
        inputGroup: '<div className="input-group">%s%s</div>',
        searchInput: '<input className="%s%s" type="text" placeholder="%s">',
        searchButton: '<button className="%s" type="button" name="search" title="%s">%s %s</button>',
        searchClearButton: '<button className="%s" type="button" name="clearSearch" title="%s">%s %s</button>'
      }
  ...
  var BootstrapTable = /*#__PURE__*/function () {
    ...
    _createClass(BootstrapTable, [{
      ...
      value: function initContainer() {
        var topPagination = ['top', 'both'].includes(this.options.paginationVAlign) ? '<div className="fixed-table-pagination clearfix"></div>' : '';
        var bottomPagination = ['bottom', 'both'].includes(this.options.paginationVAlign) ? '<div className="fixed-table-pagination"></div>' : '';
        var loadingTemplate = Utils.calculateObjectValue(this.options, this.options.loadingTemplate, [this.options.formatLoadingMessage()]); // 아랫줄 <div class=\"fixed-table-toolbar\"></div>\n 
        this.$container = $__default["default"]("\n      <div class=\" ".concat(this.constants.theme, "\">\n           ").concat(topPagination, "\n      <div class=\"\">\n      <div class=\"\"><table></table></div>\n      <div class=\"\">\n      <div class=\"\">\n      ").concat(loadingTemplate, "\n      </div>\n      </div>\n      <div class=\"\"></div>\n      </div>\n      ").concat(bottomPagination, "\n      </div>\n    "));
        ...
        this.$container.after('<div className="clearfix"></div>');
        ...
        if (this.options.height) {
          ...
          if (this.options.classes.split(' ').includes('table-bordered')) {
            this.$tableBody.append('<div className="fixed-table-border"></div>');
      ...
      value: function initHeader() {
        ...
        this.options.columns.forEach(function (columns, i) {
          var html = [];
          html.push("<tr".concat(Utils.sprintf(' className="%s"', _this2._headerTrClasses[i]), " ").concat(Utils.sprintf(' style="%s"', _this2._headerTrStyles[i]), ">"));
          ...
          columns.forEach(function (column, j) {
            var class_ = Utils.sprintf(' className="%s"', column['class']);
            ...
            if (headerStyle && headerStyle.classes) {
              classes = Utils.sprintf(' className="%s"', column['class'] ? [column['class'], headerStyle.classes].join(' ') : headerStyle.classes);
            }
            ...
            html.push("<th".concat(Utils.sprintf(' title="%s"', column.titleTooltip)), column.checkbox || column.radio ? Utils.sprintf(' className="bs-checkbox %s"', column['class'] || '') : classes || class_, Utils.sprintf(' style="%s"', halign + style + csses.join('; ')), Utils.sprintf(' rowspan="%s"', column.rowspan), Utils.sprintf(' colspan="%s"', column.colspan), Utils.sprintf(' data-field="%s"', column.field), // If `column` is not the first element of `this.options.columns[0]`, then className 'data-not-first-th' should be added.
            j === 0 && i > 0 ? ' data-not-first-th' : '', '>');
            html.push(Utils.sprintf('<div className="th-inner %s">', _this2.options.sortable && column.sortable ? 'sortable both' : ''));
            ...
            html.push(text);
            html.push('</div>');
            html.push('<div className="fht-cell"></div>');
            html.push('</div>');
            html.push('</th>');
          });
      ...
      value: function initToolbar() {
        ...
        if (typeof opts.toolbar === 'string' || _typeof(opts.toolbar) === 'object') {
          $__default["default"](Utils.sprintf('<div className="bs-bars %s-%s"></div>', this.constants.classes.pull, opts.toolbarAlign)).appendTo(this.$toolbar).append($__default["default"](opts.toolbar));
          ...
          columns: {
            ...
            html: function html() {
              ...
              if (opts.showColumnsSearch) {
                html.push(Utils.sprintf(_this4.constants.html.toolbarDropdownItem, Utils.sprintf('<input type="text" className="%s" name="columnsSearch" placeholder="%s" autocomplete="off">', _this4.constants.classes.input, opts.formatSearch())));
              ...
              if (opts.showColumnsToggleAll) {
                ...
                html.push(Utils.sprintf(_this4.constants.html.toolbarDropdownItem, Utils.sprintf('<input type="checkbox" className="toggle-all" %s> <span>%s</span>', allFieldsVisible ? 'checked="checked"' : '', opts.formatColumnsToggleAll())));
                html.push(_this4.constants.html.toolbarDropdownSeparator);
              }
      ...
      value: function initPagination() {
        ...
        if (this.paginationParts.includes('pageSize')) {
          html.push('<div className="page-list">');
      ...
      value: function initRow(item, i, data, trFragments) {
        ...
        html.push('<tr', Utils.sprintf(' %s', htmlAttributes.length ? htmlAttributes.join(' ') : undefined), Utils.sprintf(' id="%s"', Array.isArray(item) ? undefined : item._id), Utils.sprintf(' className="%s"', style.classes || (Array.isArray(item) ? undefined : item._class)), Utils.sprintf(' style="%s"', Array.isArray(item) ? undefined : item._style), " data-index=\"".concat(i, "\""), Utils.sprintf(' data-uniqueid="%s"', Utils.getItemField(item, this.options.uniqueId, false)), Utils.sprintf(' data-has-detail-view="%s"', this.options.detailView && Utils.calculateObjectValue(null, this.options.detailFilter, [i, item]) ? 'true' : undefined), Utils.sprintf('%s', data_), '>');
        ...
        this.header.fields.forEach(function (field, j) {
          ...
          if (item["_".concat(field, "_class")]) {
            class_ = Utils.sprintf(' className="%s"', item["_".concat(field, "_class")]);
          }
          ...
          if (column.checkbox || column.radio) {
            ...
            if (_this7.options.smartDisplay && value === '') {
              text = '<div className="card-view"></div>';
            }
          } else {
      ...
      value: function initFooter() {
        ...
        if (Utils.hasDetailViewIcon(this.options)) {
          detailTemplate = '<th className="detail"><div className="th-inner"></div><div className="fht-cell"></div></th>';
        }
        ...
        try {
          for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
            ...
            if (style && style.classes) {
              class_ = Utils.sprintf(' className="%s"', column['class'] ? [column['class'], style.classes].join(' ') : style.classes);
            }
            ...
            html.push('>');
            html.push('<div className="th-inner">');
            var value = '';
      ...
      value: function expandRow(index, _columnDetailFormatter) {
        ...
        $tr.after(Utils.sprintf('<tr className="detail-view"><td colspan="%s"></td></tr>', $tr.children('td').length));
