	...
	if (targetList.length > 0) {
		...
		$.each(targetList, function (idx, item) {
			...
			if (settings.columnSelector === true) {
				...
				// settings
				$(item).before('<div className="hidden"><div id="popover-target" className="' + popOverClass + ' removeFirstCol"></div></div>');
...
function AddColumnSelector(objJson) {
     ...
     *              <div id="popover-target" className="targetclass"></div>
     *          </div>
     *  3)  <th className="column-title" data-sorter="false">
	            <input id="popover" type="checkbox" className="popoverclass" data-sorter="false">
            </th>
